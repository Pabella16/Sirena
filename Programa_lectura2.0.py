...
while correr:
    ...
    console.print("[bold green]Selecciona el relato que deseas leer introduciendo un número: \n[/]")
    while not seleccion_relato:
        console.print("1.[bold green] Muñecas \n[/]")
        console.print("2.[bold green] Hambre \n[/]")
        seleccion = input()
        ...
        if seleccion.isdigit() and int(seleccion) == 1:
            ...
            console.print("[bold green]La selección no es válida, por favor introduce un número del 1 al 2. [/]")
    ...
    while elemento < len(relato):
        ...
        # no se cumplió una categoría anterior. En este caso es texto único que solamente muestra y no pasa más na'    
            ...
            console.print(relato[elemento]["contenido"], end="", justify="full")
            elemento = relato[elemento]["siguiente"]
            ...
            if relato[elemento]["tipo"] != "FIN":
                console.print("[bold green]\nENTER PARA CONTINUAR[/]", end='', justify="right")
                input()
       ...
       #TODO añadí un par de \n pa que funcione mejor en cmd, hay que ver si funciona en los .exe    
            if relato[elemento]["prompt"] != "":
               console.print(relato[elemento]["prompt"]+"\n", justify="full")
            ...
            while not decision_valida:
                ...
                console.print("[bold green]¿Qué opción escoges? " + "(1 a " + str(len(relato[elemento]["contenido"]))+ "): [/]",end="",justify="right")
                ...
                if not decision_tomada.isdigit() or int(decision_tomada)>len(relato[elemento]["contenido"]) or int(decision_tomada)<=0: 
                    console.print("[bold green]Debes escoger una de las opciones posibles introduciendo un número.[/]", end="", justify="right")
                else:
                    console.print("\n", end="")
                    ...
                    if relato[elemento]["contenido"][int(decision_tomada) - 1]["input"] != "":
                        respuesta = console.input(relato[elemento]["contenido"][int(decision_tomada) - 1]["input"])
                        console.print("\n")
        ...
        # el orden lógico, no podrá haber texto que dependa de decisiones si no se han tomado decisiones.
            ...
            console.print(relato[elemento]["contenido"]["fijo"] + variaciones[relato[elemento]["contenido"]["parte"]-1], end="", justify="full")
            elemento = relato[elemento]["siguiente"]
    ...
    while not seleccion_final:
        console.print("1.[bold green] Leer otro relato \n[/]")
        console.print("2.[bold green] Salir \n[/]")
        menu_final = input()
